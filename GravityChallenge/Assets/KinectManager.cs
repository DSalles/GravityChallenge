...
public class KinectManager : MonoBehaviour
{
    ...
    /// <summary>
    /// ////////////////////////////////////// from space hook kinect v1
    /// </summary>
    /// 

    // Minimum user distance in order to process skeleton data
    public float MinUserDistance = 1.0f;	// Maximum user distance, if any. 0 means no max-distance limitation
    public float MaxUserDistance = 0f;
   // public GameObject numPlayers;
    public GameManager GM;
    public float User1PlaceMin = 0;
    public float User1PlaceMax = 1;
    public float User2PlaceMin = -1;
    public float User2PlaceMax = 0;
    ...
    public Vector2 GetDepthMapPosForJointPos(Vector3 posJoint)
    {
      ...
      return  KinectInterop.MapSpacePointToDepthCoords(sensorData, posJoint);
    ...
    public Vector3 GetRawSkeletonJointPos(long UserId, int joint)
    {
        if (UserId == Player1ID)
            return joint >= 0 && joint < player1JointsPos.Length ? (Vector3)bodyFrame.bodyData[player1Index].joint[joint].position : Vector3.zero;  //.SkeletonPositions[joint] : Vector3.zero;
        else if (UserId == Player2ID)
            return joint >= 0 && joint < player2JointsPos.Length ? (Vector3)bodyFrame.bodyData[player2Index].joint[joint].position : Vector3.zero;
    ...
    private void ProcessBodyFrameData()
    {
        ...
        for (int i = 0; i < sensorData.bodyCount; i++)
        {
            ...
            if (userId == Player1ID)
            {
                if (Mathf.Abs(bodyPos.z) >= MinUserDistance &&
               (MaxUserDistance <= 0f || Mathf.Abs(bodyPos.z) <= MaxUserDistance) && bodyPos.x > User1PlaceMin && bodyPos.x < User1PlaceMax) // User1 is on the Right
                {
                   // player1Index = i;
                    ...
                    for (int j = 0; j < bodyData.joint.Length; j++)//(int)KinectWrapper.NuiSkeletonPositionIndex.Count; j++)
                    {
                       
                        player1JointsPos[j] = (bodyData.joint[j].position);
            ...
            {
                if (Mathf.Abs(bodyPos.z) >= MinUserDistance &&
               (MaxUserDistance <= 0f || Mathf.Abs(bodyPos.z) <= MaxUserDistance) && bodyPos.x > User2PlaceMin && bodyPos.x < User2PlaceMax) // User2 is on the Left
                {
                    // player2Index = i;
                    ...
                    for (int j = 0; j < bodyData.joint.Length; j++)//(int)KinectWrapper.NuiSkeletonPositionIndex.Count; j++)
                    {
                        ...
                        player2JointsPos[j] = (bodyData.joint[j].position);
    ...
    private void CalibrateUser(Int64 userId, int bodyIndex)
    {
        if (!Player1Calibrated)
        {
            if (!alUserIds.Contains(userId))
            {
                if (CheckForCalibrationPose(userId, bodyIndex, playerCalibrationPose))
                {
                    ...
                    Player1ID = userId;
                    Player1Index = uidIndex;
                    AllPlayersCalibrated = !TwoUsers ? alUserIds.Count >= 1 : alUserIds.Count >= 2; 
                }
            }
        }
        else if (TwoUsers && !Player2Calibrated)
        {
            if (!alUserIds.Contains(userId))
            {
                if (CheckForCalibrationPose(userId, bodyIndex, playerCalibrationPose))
                {
                    ...
                    Player2ID = userId;
                    Player2Index = uidIndex;
                    AllPlayersCalibrated = !TwoUsers ? alUserIds.Count >= 2 : alUserIds.Count >= 2;
                }
            }
        }
        ...
        {
            if (GM != null) GM.AllPlayersCalibrated();
            //Debug.Log("All prayers calibrated.");
    ...
    private void RemoveUser(Int64 userId)
    {
        if (userId == Player1ID)
        {
           ...
           //the first avatar controller should be player 1
                avatarControllers[0].ResetToInitialPosition();
        ...
        if (userId == Player2ID)
        {
          ...
          //  Player2Index = 0;
           ...
           // the second avatar controller should be 2
                avatarControllers[1].ResetToInitialPosition();
    ...
    internal Vector2 GetColorMapPosForDepthPos(Vector2 posDepth)
    {
        return KinectInterop.MapDepthPointToColorCoords(sensorData, posDepth, 0);
    }
}
